  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 px-3 py-2.5">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-neutral-400">{order.status}</span>
      </div>
...
function OrdersSection({ title, orders }) {
  return (
    <div className="space-y-2">
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-2">
          {orders.map((order) => (
            <OrderRow key={`${title}-${order.id}`} order={order} />
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 space-y-4 overflow-y-auto">
        {loading ? <p className="text-sm text-gray-500 dark:text-neutral-400">Loading orders...</p> : null}
        {error ? <p className="text-xs text-rose-600">{error}</p> : null}
        {!loading ? (
          <>
            <OrdersSection title="Active" orders={active} />
            <OrdersSection title="Past" orders={past} />
          </>
        ) : null}
      </div>
    </div>
