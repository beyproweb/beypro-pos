...
function OrderStatusHeader({ title, subtitle, meta, onBack }) {
  return (
    <header className="sticky top-0 z-[120] bg-white/95 backdrop-blur border-b border-neutral-200">
        ...
        <div className="flex items-center gap-3">
          {onBack ? (
            <button
              type="button"
              onClick={onBack}
              className="h-10 w-10 -ml-1 inline-flex items-center justify-center rounded-full hover:bg-neutral-100 active:bg-neutral-200 transition"
              aria-label="Back"
            >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-baseline justify-between gap-3">
              ...
              {meta ? (
                <div className="text-xs text-neutral-500 shrink-0">{meta}</div>
              ) : null}
            </div>
            {subtitle ? (
              <div className="text-sm text-neutral-600 truncate">{subtitle}</div>
            ) : null}
          </div>
        </div>
      </div>
        ...
        {steps.map((s, idx) => {
          ...
          return (
            <div key={s.key} className="flex-1 min-w-0">
              ...
              {idx !== steps.length - 1 ? (
                <div className="mt-2 h-[2px] w-full bg-neutral-200 rounded-full">
                  <div
                    className={[
                      "h-[2px] rounded-full",
                      isDone
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white px-4 py-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-neutral-50 border border-neutral-200 px-3 py-2">
          <div className="text-[11px] text-neutral-500">{t("Time")}</div>
          <div className="mt-0.5 text-sm font-semibold text-neutral-900">
            {timerLabel}
          </div>
        </div>
        <div className="rounded-xl bg-neutral-50 border border-neutral-200 px-3 py-2">
          <div className="text-[11px] text-neutral-500">{t("Payment")}</div>
          <div className="mt-0.5 text-sm font-semibold text-neutral-900 truncate">
            {paymentLabel}
          </div>
        </div>
        <div className="rounded-xl bg-neutral-50 border border-neutral-200 px-3 py-2">
          <div className="text-[11px] text-neutral-500">{t("Total")}</div>
          <div className="mt-0.5 text-sm font-semibold text-neutral-900">
            {totalLabel}
          </div>
        </div>
        <div className="rounded-xl bg-neutral-50 border border-neutral-200 px-3 py-2">
          <div className="text-[11px] text-neutral-500">{t("Status")}</div>
          <div className="mt-0.5 text-sm font-semibold text-neutral-900 truncate">
            {statusLabel}
          </div>
        </div>
            ...
            <li key={item.id} className="px-4 py-4">
              <div className="flex items-start gap-3">
                <div className="shrink-0 w-8 text-center">
                  <div className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-neutral-100 text-neutral-900 text-sm font-semibold">
                    {quantity}
                  </div>
                </div>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 flex-wrap">
                        ...
                        {isItemPaid ? (
                          ...
                          <span className="text-[11px] px-2 py-0.5 rounded-full border bg-neutral-50 text-neutral-500 border-neutral-200">
                            {t("Unpaid")}
                          </span>
                        )}
                      </div>
...
function StickyBottomActions({
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-[130] border-t border-neutral-200 bg-white/95 backdrop-blur">
      <div className="mx-auto w-full max-w-[640px] px-4 py-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {onSecondary ? (
              <button
                type="button"
                onClick={onSecondary}
                className="h-11 px-4 rounded-xl border border-neutral-300 text-neutral-800 bg-white hover:bg-neutral-50 active:bg-neutral-100 transition font-semibold"
              >
                {secondaryLabel}
              </button>
            ) : null}
