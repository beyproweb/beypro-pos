  ...
  const toggleAutoOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(result.error || "Failed to update auto order status");
      }
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col justify-between mt-2">
      {cartInfo && (
        <div className="mt-4 text-sm bg-white p-3 rounded-lg shadow-inner flex-1">
          ...
          <p><strong>{t("Next")}:</strong> {cartInfo.scheduled_at ? new Date(cartInfo.scheduled_at).toLocaleString("tr-TR", { hour12: false }) : "—"}</p>
          <p><strong>{t("Repeat")}:</strong> {cartInfo.repeat_type || t("none")}</p>
          <p><strong>{t("Items")}:</strong> {cartInfo.items?.length || 0}</p>
        </div>
      )}
          ...
          <ul className="space-y-1 max-h-48 overflow-y-auto">
            {(showAllHistory ? history : history.slice(0, 5)).map((cart) => (
              <li key={cart.id} className="border-b py-1">
                ...
                {cart.skipped ? (
                  <span className="text-red-500 italic">{t("No order sent (stock OK)")}</span>
                ) : cart.items?.length > 0 ? (
                  cart.items.map(
      ...
      {loading && <p className="text-xs text-gray-400 mt-2">{t("Updating...")}</p>}
    </div>

    {/* Spacer ensures button is always at the bottom */}
    <div className="flex-grow" />

    {/* Open Cart Button — always bottom-aligned */}
    <button
      onClick={() => openSupplierCart(supplier.id)}
