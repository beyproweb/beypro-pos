...
// Purpose: Electron/native Windows print (if available),
//          Bridge download links & ping (web fallback),
...
const isElectron = typeof window !== "undefined" && !!window.beypro;

// --- ESC/POS bridge helpers (for IP printing via bridge) ---
...
let BRIDGE_DOWNLOAD_ORIGIN = (import.meta?.env?.VITE_BRIDGE_DOWNLOAD_ORIGIN || window.location.origin).replace(/\/$/, "");
...
export default function PrinterTab() {
  ...
  const handleTestPrint = async () => {
    try {
      ...
      if (!ip) {
        alert("Please set a printer IP first.");
        return;
      }
...
// --- Bridge/Electron status ping ---
const pingBridge = async () => {
  if (isElectron) {
    try {
      const j = await window.beypro.getInfo();
      setBridgeInfo(j);
    } catch (e) {
      setBridgeInfo({ ok: false, error: String(e?.message || e) });
    }
    return;
  }
  try {
 ...
 // --- Windows printers: Electron native or Bridge fallback ---
const refreshWindowsPrinters = async () => {
  ...
  useEffect(() => {
    pingBridge();
    if (!isElectron) {
      // In web mode, a bridge might be available; try to scan USB on load
      refreshUsb();
    }
  ...
  // Auto-load Windows printers when on Windows (electron or bridge says win)
  useEffect(() => {
    const plat = String(bridgeInfo?.platform || "").toLowerCase();
  ...
  // --- Windows spooler print (Electron native or Bridge fallback) ---
// --- Windows spooler print (Electron native or Bridge fallback) ---
const handlePrintWindows = async () => {
  ...
  try {
    ...
    if (isElectron) {
      const r = await window.beypro.printRaw({ printerName: selectedWin, dataBase64 });
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      {/* Step 1 — Download & Install Bridge (hidden in Electron mode) */}
      {!isElectron && (
        <div className="rounded-2xl border p-4 bg-white/70 space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Install the bridge on the computer where your USB printer is plugged in. It will allow this page to talk to
            the printer at <code>http://127.0.0.1:7777</code>.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <a
              href={`${BRIDGE_DOWNLOAD_BASE}/beypro-bridge-win-x64.zip?v=${BRIDGE_VER}`}
              className="px-4 py-3 rounded-2xl bg-blue-700 text-white font-bold text-center shadow hover:bg-blue-800"
              download
            >
              Windows
            </a>
            <a
              href={`${BRIDGE_DOWNLOAD_BASE}/beypro-bridge-mac-x64.tar.gz?v=${BRIDGE_VER}`}
              className="px-4 py-3 rounded-2xl bg-gray-800 text-white font-bold text-center shadow hover:bg-gray-900"
              download
            >
              macOS (Apple Silicon via Rosetta)
            </a>
            <a
              href={`${BRIDGE_DOWNLOAD_BASE}/beypro-bridge-mac-x64.tar.gz?v=${BRIDGE_VER}`}
              className="px-4 py-3 rounded-2xl bg-neutral-900 text-white font-bold text-center shadow hover:bg-neutral-950"
              download
            >
              macOS (Intel/Rosetta)
            </a>
            <a
              href={`${BRIDGE_DOWNLOAD_BASE}/beypro-bridge-linux-x64.tar.gz?v=${BRIDGE_VER}`}
              className="px-4 py-3 rounded-2xl bg-zinc-900 text-white font-bold text-center shadow hover:bg-zinc-950"
              download
            >
              Linux
            </a>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Apple-silicon Macs may need Rosetta to run the x64 build:{" "}
          ...
          <button onClick={handleTestPrint} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg">
            Test Print
          </button>
        </div>
      )}

      {/* In Electron, show a compact status card */}
      {isElectron && (
        <div className="rounded-2xl border p-4 bg-white/70 space-y-2">
          <div className="font-semibold">App Status</div>
          {bridgeInfo?.ok ? (
            <div className="text-sm text-emerald-700">
              🖥️ Electron Mode — <span className="font-mono">{bridgeInfo.platform}</span> — v
              <span className="font-mono">{bridgeInfo.version}</span>
            </div>
          ) : (
            <div className="text-sm text-rose-700">Electron runtime detected, but status unavailable.</div>
          )}
      ...
      {/* Windows printers section (both Electron and Bridge on Windows) */}
      {String(bridgeInfo?.platform || "").toLowerCase().includes("win") && (
        <div className="rounded-2xl border p-4 bg-white/70 space-y-3">
              ...
              {winPrinters.length > 0 ? (
                ...
                >
                  ...
                  {winPrinters.map((p, i) => (
                    <option key={`${p.name}:${i}`} value={p.name}>
                      {p.isDefault ? "⭐ " : ""}
                      {p.name} {p.driver ? `— ${p.driver}` : ""} {p.port ? `(${p.port})` : ""}
                    </option>
                  ))}
                </select>
              ) : (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    {t(
                      "List is empty. Enter the exact printer name from Control Panel → Devices and Printers, then click “Use This Name”."
                    )}
                  </p>
                  <div className="flex gap-2">
                    <button
      ...
      <div className="text-xs text-gray-500 space-y-1">
        {!isElectron && <div>• Windows: install the printer’s USB driver if detection fails.</div>}
        {!isElectron && (
          <div>
        ...
        {isElectron && <div>• Desktop mode uses native Windows spooler (no separate Bridge needed).</div>}
      </div>
    </div>
  );
}
