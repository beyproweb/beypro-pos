// QrMenu.jsx — Beypro QR Menu with Real-Time Order Status
...
let socket;
function useSocketIO(onOrderUpdate, orderId) {
  useEffect(() => {
    if (!orderId) return;
    if (!socket) {
      // Use CDN or window.io, or your preferred method
    ...
    const updateHandler = data => {
      if (!data) return;
      // If update relates to our orderId, trigger update
      if (Array.isArray(data.orderIds) && data.orderIds.includes(orderId)) {
        onOrderUpdate && onOrderUpdate();
      }
      // Some events are broadcasted as just "orders_updated"
...
// --- Existing language, modal, and utility code omitted for brevity (copy from your QrMenu.jsx) ---

// --- ORDER STATUS SCREEN ---
function OrderStatusScreen({ orderId, table, onOrderAnother, onAllDelivered }) {
  ...
  const fetchOrder = async () => {
    ...
    // Items API is already nested or separate
    let orderItems = [];
...
// --- MAIN QR MENU COMPONENT ---
export default function QrMenu() {
  // ... (same state as your original: orderType, table, cart, etc)
  ...
  useEffect(() => { localStorage.setItem("qr_cart", JSON.stringify(cart)); }, [cart]);

  // Fetch initial data (categories, products, extras, tables)
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/extras-groups`).then(res => res.json()).then(data => setExtrasGroups(data));
    fetch(`${API_URL}/api/orders`).then(res => res.json()).then(orders => {
      const occupied = orders.filter(order => order.table_number && order.status !== "closed").map(order => Number(order.table_number));
  ...
  // After order is sent and orderId exists, show order status screen
  if (orderId && showStatus) {
    return (
      <OrderStatusScreen
        orderId={orderId}
        table={table}
