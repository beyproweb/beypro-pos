...
export default function QrMenuSettings() {
  ...
  useEffect(() => {
    // ✅ secureFetch already returns parsed JSON
    secureFetch("/products")
      .then(setProducts)
      .catch((err) => console.error("❌ Failed to load products:", err));

    secureFetch("/settings/qr-menu-disabled")
      .then((data) => {
        if (Array.isArray(data)) setDisabledIds(data);
        else if (typeof data === "string") {
          try {
            setDisabledIds(JSON.parse(data) || []);
          } catch {
            setDisabledIds([]);
          }
  ...
  const toggleDisable = async (productId) => {
    ...
    try {
      ...
      console.error("❌ Failed to save QR menu settings:", err);
