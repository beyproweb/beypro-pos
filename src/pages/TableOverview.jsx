...
const groupedTables = tables.reduce((acc, tbl) => {
  ...
  // --- RETURN (NEW UI) ---
  const tabHref = (tabId) => `/tableoverview?tab=${encodeURIComponent(tabId)}`;

  return (
    <div className="min-h-screen bg-transparent px-0 pt-4 relative">
      {/* Compact in-page tabs (avoid header interference) */}
      <div className="sticky top-0 z-10 bg-slate-50/80 dark:bg-zinc-950/70 backdrop-blur border-b border-slate-200/60 dark:border-zinc-800/60">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition ${
                    (isActive && location.search.includes(`tab=${tab.id}`)) || activeTab === tab.id
